...
export class Favorites {
  constructor(root) {
    ...
    this.API_URL = "https://filmov-api.onrender.com";
...
export class FavoritesView extends Favorites {
    ...
    saveReviewBtn.onclick = async () => {
      ...
      try {
        if (this.activeDrawerMovieNoteId) {
          ...
          const res = await fetch(
            `${this.API_URL}/movie_notes/${this.activeDrawerMovieNoteId}`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
        ...
        const updatedNotes = this.allActiveNotes.filter(
          (n) => n.imdb_id === this.activeDrawerMovie.imdb_id,
        );
        this.activeDrawerMovie.notes = updatedNotes;
        this.activeDrawerMovie.avgRating = this.getMovieAvgRating(
          this.activeDrawerMovie.imdb_id,
        );
  ...
  openDrawer(movie) {
    ...
    document.getElementById("drawer-avg-rating").textContent =
      movie.avgRating || this.getMovieAvgRating(movie.imdb_id);
    ...
    const notesList =
      movie.notes ||
      this.allActiveNotes.filter((n) => n.imdb_id === movie.imdb_id);
  ...
  updateProfileHeader() {
    ...
    if (this.currentUser) {
      const avatarUrl =
        ...
        `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(this.currentUser.name)}`;
    ...
    if (this.activeTab === "all") {
      if (this.currentUser) {
        ...
        const alreadyFavorited = userNotes.some(
          (n) => n.user_id === this.currentUser.id,
        );
